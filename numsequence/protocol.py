...
def init_decoder(remaining_msg):
    ...
    msg_info = re.search(
        r"([0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}),([0-9]+),([0-9]+)",
        remaining_msg,
    )
    if msg_info is None:
...
MAX_MSG_PREAMBLE = (
    max([len(x) for x in MSG_DECODERS.keys()]) + 1 + len(f"{sys.maxsize}") + 1
)
...
class ProtocolEncoder:
    def _decode_message(self, message):
        ...
        return MSG_DECODERS.get(msg_info.group(1), unknown_message_decoder)(
            msg_info.group(2)
        )
